...
from dso.program import convert_to_tokens_traversal
...
def learn(sess, controller, pool, gp_controller, output_file,
          ...
          const_optimizer="scipy", const_params=None, Ho=None, w1=1.0, w2=0.0, w3=0.0, 
          lowerD_const=None, lowerD_var=None, alpha=0.5,
          epsilon=None, n_cores_batch=1, verbose=True, save_summary=False,
    ...
    # run_gp_meld = False
    # run_Ho_gp = gp_controller is not None  # 
    ...
    if use_memory:
        ...
        programs = [from_tokens(a, Ho, w1, w2, w3, lowerD_const, lowerD_var) for a in actions]
    ...
    for epoch in range(n_epochs):
        ...
        actions_n_Ho = actions
        max_token = np.max(actions)

        # Add Side Information Expression to actions --   right now added 3 dupliated copies of Ho
        if Ho is not None:
            Ho_tokens = convert_to_tokens_traversal(Ho)
            padlength = controller.max_length - len(Ho_tokens)
            for i in range(3):
                Ho_tokens_i = np.pad(Ho_tokens, (0, padlength), 'constant', constant_values=(0,i))
                actions_n_Ho = np.insert(actions_n_Ho, 0, values=Ho_tokens_i, axis=0)

        programs = [from_tokens(a, Ho, w1, w2, w3, lowerD_const, lowerD_var) for a in actions]
        nevals += batch_size

        # if run_Ho_gp:
        #     print("-------- train.py run_Ho_gp -------------")
        #     deap_programs, deap_actions, deap_obs, deap_priors = gp_controller(controller.Ho_traversal)
        ...
        #     print("-------- train.py run_Ho_gp end-------------")

        # Run GP seeded with the current batch, returning elite samples
        if run_gp_meld:
            # print("-------- train.py run_gp_meld -------------")
            # deap_programs, deap_actions, deap_obs, deap_priors = gp_controller(actions, Ho, w1, w2, w3)
            deap_programs, deap_actions, deap_obs, deap_priors = gp_controller(actions_n_Ho, Ho, w1, w2, w3, lowerD_const, lowerD_var)
            # nevals += gp_controller.nevals   #  Comment this, because we want to run full batch for expression generator
            # Combine RNN and deap programs, actions, obs, and priors
            programs = programs + deap_programs
            actions = np.append(actions, deap_actions, axis=0)
            obs = np.append(obs, deap_obs, axis=0)
        ...
        #   add the post filter to filter the expressions.
        post_filter = "div"
        post_filter = None
        if post_filter is not None:
            post_filter_token = Program.library[Program.library.names.index(post_filter.lower())]
            post_token_count = np.array([Counter(p.traversal)[post_filter_token] for p in programs])
            # keep = []
            # for i in range(len(programs)):
            #     if Counter(programs[i].traversal)[post_filter_token] == 1:
            #         # print(programs[i].traversal)
            #         keep.append(i)
            keep = post_token_count == 1
            ...
            # print ("======= train.py traversal")
            # print(post_filter_token)
            # print(keep)
            # print(programs)
        ...
        # print ("======= train.py r, r_best")
        # print(r)
        # print(r_max)
        # print(r_best)

        #   considers the case of epsilon=1.0 or None to turn of risk-seeking. Sometimes risk-seeking will lead the sample actions to be empty.
        quantile = np.quantile(r, 0, interpolation="higher")
