...
class Controller(object):
    ...
    def __init__(self, sess, prior, state_manager,                  
                 # Side Information Expression
                 Ho=None,
                 input_var_num=4,
                 debug=0, summary=False,
        ...
        Ho_traversal = None
        if Ho is not None:
            Ho_traversal = convert_to_tokens_traversal(Ho)
            Ho_traversal = np.array(Ho_traversal)
            Ho_traversal = Ho_traversal.astype(np.float32)
            self.Ho_traversal = Ho_traversal
            Ho_length = len(Ho_traversal)
        ...
        with tf.name_scope("controller"):
            ...
            # Create encoder
            if Ho_traversal is not None:
                encoder_cell = tf.contrib.rnn.MultiRNNCell(
                        [make_cell(cell, n, initializer=initializer) for n in num_units])
                Ho_traversal = tf.broadcast_to(Ho_traversal, [self.batch_size, Ho_length])
                Ho_traversal = tf.reshape(Ho_traversal, [self.batch_size, Ho_length, 1])
                # print("------controller.py Ho_traversal-------")
                # print(Ho_traversal)
            ...
            # initial_obs = task.set_side_information_task(prior)   #      
            ...
            # print("=======controller.py initial_obs ======")
            # print(initial_obs)
            ...
            def loop_fn(time, cell_output, cell_state, loop_state):
                ...
                if cell_output is None: # time == 0
                    ...
                    if Ho_traversal is not None:
                        print("========controller.py Ho_traversal ========")
                        print(Ho_traversal)
                        next_cell_state = encoder_last_state   # 
                    ...
                    ##  
                    # numlines=self.batch_size
                    # numcolunms=logits.shape[1].value
                    # div_column = input_var_num
                    # columnTensor=-np.inf*tf.ones([self.batch_size, 1], tf.float32)
                    # _,_,logits=modify_one_column(logits,columnTensor,div_column,numlines,numcolunms)
                    # print("======= controller.py logits ========")
                    # print(columnTensor)
                    # print(logits)
                    # print(actions_ta)
                    ...
                    if time == 1:
                        next_actions_ta = actions_ta.write(0, tf.cast(4, dtype=tf.int32))
            ...
            with tf.variable_scope('policy'):
                ...
                actions_ta, obs_ta, priors_ta, obs, prior, lengths, finished = loop_state
                # print("======= controller.py next_priors_ta, prior ========")
                # print(priors_ta)
                # print(prior)
    ...
    def sample(self, n):
        ...
        init_obs, actions, obs, priors = self.sess.run([self.init_obs, self.actions, self.obs, self.priors], feed_dict=feed_dict)

        # print("======controller.py actions, init_obs, feed_dict======")
        # print('-------init_obs')
        # print(init_obs)
        # print('-------actions')
        # print(actions)
