...
def convert_to_tokens_traversal(Ho):
    # Convert str to list of str
    ...
    if isinstance(Ho, list):
        ...
        for s in Ho:
            if s in Program.library.names:
                ...
                assert "const" not in Ho, "Currently does not support both placeholder and hard-coded constants."
                t = Program.library.const_token
                constants.append(float(s))
                print("=======program.py constants =======")
                print(constants)
            else:
                raise ValueError("Did not recognize token {}.".format(s))
            traversal.append(t)
...
def from_tokens(tokens, Ho, w1, w2, w3, lowerD_const, lowerD_var, skip_cache=False, on_policy=True, finish_tokens=True):
    ...
    if skip_cache or Program.task.stochastic:
        p = Program(tokens, Ho, w1, w2, w3, lowerD_const, lowerD_var, on_policy=on_policy)
    else:
        key = tokens.tostring() 
        try:
            p = Program.cache[key]
...
class Program(object):
    ...
    Ho : Side Information expression (list of string)
    ...
    def __init__(self, tokens=None, Ho=None, w1=1.0, w2=0.0, w3=0.0, lowerD_const=None, lowerD_var=None, on_policy=True):
        """
        ...
        if tokens is not None:
            self._init(tokens, Ho, w1, w2, w3, lowerD_const, lowerD_var, on_policy)
            
    def _init(self, tokens, Ho, w1, w2, w3, lowerD_const, lowerD_var, on_policy=True):
        ...
        self.lowerD_const = None
        self.lowerD_var = None

        if lowerD_const is not None:
            lowerD_const_token = Program.library.names.index(lowerD_const)
            lowerD_var_token = Program.library.names.index(lowerD_var)
            self.lowerD_const = Program.library[lowerD_const_token]
            self.lowerD_var = Program.library[lowerD_var_token]

        self.Ho = Ho
        Ho_tokens_index = None
        self.Ho_traversal = None
        if Ho is not None:
            Ho_tokens_index = convert_to_tokens_traversal(Ho)
            self.Ho_traversal = [Program.library[t] for t in Ho_tokens_index]
        self.w1 = w1
        self.w2 = w2
        self.w3 = w3
        # print("======= program.py Ho_traversal =======")
        # print(self.Ho_traversal)
    ...
    def Ho_execute(self, Ho_traversal, X):
        if not Program.protected:
            result, _, _, _ = Program.execute_function(Ho_traversal, X)
        else:
            result = Program.execute_function(Ho_traversal, X)
        return result
    ...
    def optimize(self):
        ...
        def f(consts):
            ...
            r = self.task.reward_function(self)[0]
    ...
    #   defined bound optimize
    def bound_optimize(self):
        """
        Optimizes PlaceholderConstant tokens against the reward function. The
        optimized values are stored in the traversal.
        """
        ...
        #   Edited the optimization constant with bound
        bounds_list = [(-1, 1) for i in range(len(self.const_pos))]
        # optimized_constants = minimize(f, x0, bounds=None, options={'maxiter':10})['x']
        optimized_constants = minimize(f, x0, options={'maxiter':5})['x']
        # optimized_constants = Program.const_optimizer(f, x0)

        # print ("====== program.py optimized_constants ====")
    ...
    def r(self):
        ...
        with warnings.catch_warnings():
            ...
            # self.optimize()
            self.bound_optimize()
            # [r_value, self.NRMSE, self.X_train, self.y_hat, self.curr_expr]= self.task.reward_function(self)
            [r_value, self.NRMSE]= self.task.reward_function(self)
    ...
    def print_stats(self):
        ...
        print("\tSymplified Expression: {}".format(self.sympy_expr))
