...
def DREGS():
	config_json = os.path.join("config/", "DREGS_benchmark_regression-Entropy-Max.json")
	model = DeepSymbolicOptimizer(config_json)
	model.train()

def DSR():
	config_json = os.path.join("config/", "DSR_benchmark_regression-Entropy-Max.json")
	model = DeepSymbolicOptimizer(config_json)
	model.train()
...
def main():
	...
	new_config_json = "./config/benchmark_regression-N-Distribution.json"

	benchmark = "N-Distribution"
	ground_truth = "mul,div,1.0,sqrt,mul,2.0,3.14159,exp,neg,div,n2,x1,2.0"
